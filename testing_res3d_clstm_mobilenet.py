...
if cfg_dataset==JESTER:
  if cfg_type==SEPARABLE:
    pretrained_model = '%s/jester_rgb_separableclstm_weights.h5'%model_prefix
  elif cfg_type==GROUP:
    pretrained_model = '%s/jester_rgb_groupclstm_weights.h5'%model_prefix
  elif cfg_type==SHUFFLE:
    pretrained_model = '%s/jester_rgb_shuffleclstm_weights.h5'%model_prefix
  elif cfg_type==GATED:
    pretrained_model = '%s/jester_rgb_gatedclstm_weights.h5'%model_prefix
elif cfg_dataset==ISOGD:
  if cfg_type==SEPARABLE:
    pretrained_model = '%s/isogr_rgb_separableclstm_weights.h5'%model_prefix
  elif cfg_type==GROUP:
    pretrained_model = '%s/isogr_rgb_groupclstm_weights.h5'%model_prefix
  elif cfg_type==SHUFFLE:
    pretrained_model = '%s/isogr_rgb_shuffleclstm_weights.h5'%model_prefix
  elif cfg_type==GATED:
    pretrained_model = '%s/isogr_rgb_gatedclstm_weights.h5'%model_prefix
